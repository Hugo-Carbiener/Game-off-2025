...
static func parse(amount_string :String, possible_tiles_string : String, range_string : String) -> TileAmountRequirement:
	return TileAmountRequirement.new(int(amount_string), possible_tiles_string.split(Constants.TILE_REQUIREMENT_TILES_SEPARATOR), int(range_string));

func is_met(tilemap_position : Vector2i) -> bool:
	var offset_coordinates_list = MainTilemap.get_neighbor_tile_coordinate_offset_at_range(tile_range);
	var counter = 0;
	for offset_coordinates in offset_coordinates_list:
		var tile_position_to_consider = tilemap_position + offset_coordinates;
		if !MainTilemap.instance.tiles.has(tile_position_to_consider): continue;
...
func get_requirement() -> Dictionary[Vector2i, String]:
	...
	var possible_coordinates = MainTilemap.get_neighbor_tile_coordinate_offset_at_range(tile_range);
