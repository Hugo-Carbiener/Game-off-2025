...
@export var cursor_preview : Sprite2D; ## the control we create to visually move around while dragging
...
func get_snapped_tile_position(local_position : Vector2) -> Vector2:
	...
	var snaped_card_position = MainTilemap.instance.map_to_local(MainTilemap.instance.local_to_map(card_position)) - Vector2(TileDataManager.tile_size / 2) * Vector2.LEFT;
	return snaped_card_position;

func get_conditionnal_snapped_tile_position() -> Vector2:
	# Add an offset so the mouse is on the card sprite
	var preview_position = get_local_mouse_position();
	...
	if MainTilemap.instance.is_valid_cell(tilemap_card_position):
		var snaped_card_position = MainTilemap.instance.map_to_local(tilemap_card_position) + tilemap_offset;
		return snaped_card_position;
	else:
