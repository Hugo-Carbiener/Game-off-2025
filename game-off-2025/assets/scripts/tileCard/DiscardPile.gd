...
func discard_from_tile(card_id : String, global_tile_position : Vector2):
	var tile_data = TileDataManager.tile_dictionnary[card_id];
	await ElementMovementAnimation.launch_element_movement_animation(tile_data.get_texture_region(), global_tile_position, global_position + size / 2, self);
	AnimationUtils.bounce(self, 1.5);
